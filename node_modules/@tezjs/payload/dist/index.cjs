...
const sharp__namespace = /*#__PURE__*/_interopNamespace(sharp);
        ...
        writer.on("close", () => {
          if (!error) {
            if (common.commonContainer.tezConfig.image?.enableWebPConversion) {
              sharp__namespace(filePath).webp().toBuffer().then((t) => {
                sharp__namespace(t).toFile(filePath.replace(`.${fileExtension}`, ".webp"));
                resolve(true);
              });
            } else
              resolve(true);
...
class BaseGenerator {
  constructor(redirectRoute, sitemap, globWriter) {
...
class GlobWriter {
  constructor() {
    this.pathResolver = new PathResolver();
    const globJson = common.readFileSync(this.pathResolver.globJsonPath) || { components: [], layouts: [] };
  ...
  write() {
    common.writeFileSync(this.pathResolver.globJsonPath, { components: this.components, layouts: this.layouts });
  }
}
  ...
  async generate(routePath) {
    if (common.commonContainer.tezConfig.strapi) {
      if (common.commonContainer.tezConfig.strapi.customPayloadGenerator)
        await common.commonContainer.tezConfig.strapi.customPayloadGenerator({ routePath });
      else
