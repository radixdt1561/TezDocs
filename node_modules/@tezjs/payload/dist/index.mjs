...
class GlobWriter {
  constructor() {
    this.pathResolver = new PathResolver();
    const globJson = readFileSync(this.pathResolver.globJsonPath) || { components: [], layouts: [] };
  ...
  write() {
    writeFileSync$1(this.pathResolver.globJsonPath, { components: this.components, layouts: this.layouts });
  }
}
  ...
  async generate(routePath) {
    if (commonContainer.tezConfig.strapi) {
      if (commonContainer.tezConfig.strapi.customPayloadGenerator)
        await commonContainer.tezConfig.strapi.customPayloadGenerator({ routePath });
      else
