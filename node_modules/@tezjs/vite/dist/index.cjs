...
function getImportStatement(name, path) {
  return `import ${snakeToCamel(name)} from ${common.commonContainer.buildOptions.commandName === "build" ? `"/assets/${name}.${path === "components" ? "component" : "layout"}.js"` : `"/@/${path}/${name}.vue"`};`;
}
...
class JsCodeGen extends common.PayloadReader {
  ...
  gen() {
    ...
    const preFile = common.getPath([this.commonPath.getPath([writePath, this.route.fPath]), "pre.js"]);
    const preloadFile = common.getPath([this.commonPath.getPath([writePath, this.route.fPath]), "preload.js"]);
    ...
    if (this.isPostCode) {
      const postFile = common.getPath([this.commonPath.getPath([writePath, this.route.fPath]), "post.js"]);
  ...
  preCode() {
    return depsCodeTemplate({
      ...
      postScript: this.isPostCode ? common.commonContainer.buildOptions.commandName === "dev" ? `/tez/deps${getUrl(this.route.path)}/post` : "./post.js" : ""
    }, this.route);
  }
  postCode() {
    return depsCodeTemplate({
...
class Seo extends JsCodeGen {
  constructor(route) {
    super(route);
    this.htmlMeta = { ...common.commonContainer.tezConfig.htmlMeta, ...{ head: this.tags } };
...
class HtmlPage extends Seo {
  ...
  buildHtml() {
    this.buildElements();
    var html = common.commonContainer.buildOptions.commandName === "dev" ? indexTemplate(this) : htmlMinifier.minify(indexTemplate(this), {
      minifyJS: true,
      collapseWhitespace: true,
      removeComments: true,
      removeEmptyAttributes: true,
...
class HtmlGen {
    ...
    for (var route of this.routes) {
      ...
      let page = {
        ...
        body: {
          ...
          script: !common.commonContainer.tezConfig.build.inLineJs ? [{ src: TEZJS_PATH }] : [],
          style: !common.commonContainer.tezConfig.build.inLinCss ? this.getCssRef(path) : []
        }
      };
  ...
  async getInlineJs(path) {
    ...
    let tezjsCode = common.readFileSync(common.getPath([this.commonPathResolver.distPath, TEZJS_PATH]), true);
  ...
  return {
    ...
    resolve: {
      alias: [
        ...
        { find: "/@", replacement: common.commonContainer.buildOptions.rootDir },
...
async function createViteServer() {
  let config = vite.mergeConfig(VITE_SERVER_CONFIG(), { root: common.commonContainer.buildOptions.rootDir });
...
function globTemplate(pathResolver) {
  ...
  globWriter.components.forEach((name) => {
    ...
    inputOptions[`${name}.component`] = common.getPath([pathResolver.componentsPath, `${name}.vue`], false);
  });
  globWriter.layouts.forEach((name) => {
    layoutsString += `"${name}.layout": import("/layouts/${name}.vue"),`;
    inputOptions[`${name}.layout`] = common.getPath([pathResolver.layoutsPath, `${name}.vue`], false);
  });
  return {
