import { getPath, commonContainer, readFileSync, writeFileSync, subText, CommonPathResolver, PayloadReader } from '@tezjs/common';
...
import { mergeConfig, createServer, build as build$2 } from 'vite';
...
const SERVICE_WORKER_JS_PATH = () => getPath([commonContainer.buildOptions.rootDir, "node_modules", "@tezjs", "vite", "templates", "sw._js"], false);
const TZ_JS_PATH = () => getPath([commonContainer.buildOptions.rootDir, "node_modules", "@tezjs", "vite", "templates", "tz-web-worker.js"], false);
...
const TEZJS_PATH = "assets/tez.js";
const TEZCSS_PATH = "assets/tez.css";
...
function snakeToCamel(value) {
  if (!/[_-]/.test(value))
    return value;
  return value.replace(/[0-9]/g, BLANK).toLowerCase().replace(/([-_])([a-z])/g, (_match, _p1, p2) => p2.toUpperCase());
}
...
function depsCodeTemplate(page, route, isPre = true) {
  let components = ``;
  let masterPage = "";
  let componentRefs = `{`;
  let masterPageRefs = `{`;
...
function getPreloadCodeTemplate(preloads) {
...
function postScriptPreloadCodeTemplate(preloads) {
    ...
    if (!this.importState) {
      ...
      ["store"].forEach((key) => {
        if (existsFilesorFolders[key])
          refrenceState.imports += `import * as ${key} from '#${key}';`;
      });
...
class HtmlPage extends Seo {
  ...
  buildHtml() {
    this.buildElements();
    var html = commonContainer.buildOptions.commandName === "dev" ? indexTemplate(this) : minify(indexTemplate(this), {
      minifyJS: true,
      collapseWhitespace: true,
      removeComments: true,
      removeEmptyAttributes: true,
...
let exampleOnResolvePlugin = {
  name: "example",
  setup(build2) {
    build2.onResolve({ filter: /component|layout/ }, (args) => {
...
class HtmlGen {
  ...
  async build() {
    for (var route of this.routes) {
      ...
      let page = {
        head: {
          inlineStyle: commonContainer.tezConfig.build.inLinCss ? this.getInlineCss(path) : new Array(),
  ...
  async getInlineJs(path) {
    ...
    let tezjsCode = readFileSync(getPath([this.commonPathResolver.distPath, TEZJS_PATH]), true);
          ...
          if (outputOption.importedBindings[key].length > 0) {
            ...
            dependency.js.forEach((item) => {
              if (deps.js.filter((x) => x === item).length === 0 && this.excludes.filter((x) => x === item).length === 0)
                path === TEZJS_PATH ? this.excludes.push(item) : deps.js.push(item);
            });
          } else
...
async function build() {
  ...
  let viteConfig = mergeConfig(VITE_SERVER_CONFIG(), { ...userConfig, ...{ mode: "production" } });
...
export { build, defineTezConfig, tez, tzServer };
