...
function _genTypes(schema, spaces, opts) {
  ...
  for (const key in schema.properties) {
    ...
    if (val.tsType) {
      buff.push(`${utils.escapeKey(key)}${opts.partial ? "?" : ""}: ${val.tsType},
`);
    } else if (val.type === "object") {
      buff.push(`${utils.escapeKey(key)}${opts.partial ? "?" : ""}: {`, ..._genTypes(val, spaces + " ", opts), "},\n");
    } else {
      let type;
      if (val.type === "array") {
      ...
      buff.push(`${utils.escapeKey(key)}${opts.partial ? "?" : ""}: ${type},
`);
    }
  }
  if (buff.length) {
