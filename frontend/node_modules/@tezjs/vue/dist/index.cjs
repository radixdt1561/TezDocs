...
const componentState = new class {
  constructor() {
    ...
    this.currentRoute = vue.shallowRef({ params: {} });
  }
  defineGlobalProps(app) {
    ...
    Object.defineProperty(app.config.globalProperties, "$tezRoute", {
      enumerable: true,
      get: () => vue.unref(this.currentRoute)
    });
  }
...
const TezSlot = vue.defineComponent({
  data() {
    return {
      scrollFunction: null,
      isScrolled: false,
  ...
  methods: {
    ...
    async goToNextComponent(isContinue) {
      if (!this.observer && !js.isBot()) {
        this.subscribeLazy();
        this.isInView = isContinue;
        if (!this.isInView)
          return;
      }
      this.isInView = js.isBot() || this.isInView;
      if (this.isInView) {
        let components = this.getSlotComponents(this.slotName, this.slotCategory);
        let _components = [];
        if (this.nextIndex > this.getMaxPreComponentCount() && this.postScript || this.postScript && js.isBot())
          await this.loadPostScript();
        if (components.length > this.nextIndex && !this.components[this.nextIndex]) {
          let increCount = js.isBot() ? components.length : this.nextIndex == this.getMaxPreComponentCount() + 1 ? components.length : this.nextIndex + 1;
          ...
          for (var i = startIndex; i < increCount; i++) {
            ...
            if (tezPages.components[componentName]) {
              ...
              if (increCount !== components.length)
                js.idleCallback(() => this.goToNextComponent(), { timeout: 0 });
            }
          }
    ...
    getComponentName(component) {
      return js.isMobile() && component.mobileComponentName ? component.mobileComponentName : component.name;
    }
  },
  render() {
    ...
    for (let component of this.components) {
      ...
      if (!vNode)
        vNode = cacheState.cacheVNode(component.id, vue.h(tezPages.components[componentName], { data: component.data }));
      vNodes.push(vue.h(vue.KeepAlive, { key: `${getCurrentUrl()}${component.itemName}` }, vNode));
    }
    if (!this.lazyRef)
      this.lazyRef = vue.h("div", { ref: "divLazy", style: { "height": "2px" } }, "");
    vNodes.push(this.lazyRef);
    return vue.h("div", vNodes);
  }
});

const TezIndex = vue.defineComponent({
  render() {
    return vue.h(TezSlot);
  }
});
...
const TezLazy = vue.defineComponent({
  data() {
    return {
      items: [],
      lazyRef: null,
  ...
  render() {
    ...
    if (this.$slots && this.$slots.default && this.items && this.items.length > 0)
      vNodes.push(vue.h("div", { class: this.class }, this.$slots?.default({
        items: this.items
      })));
    if (!this.lazyRef)
      this.lazyRef = vue.h("div", { ref: "divLazy" }, "");
    vNodes.push(this.lazyRef);
    return vue.h("div", vNodes);
  }
});

const TezPage = vue.defineComponent({
  data() {
    return {
      state: {},
      vNode: vue.h("div")
  ...
  render() {
    if (this.activePageUrl && this.activePageComponent && !this.state[this.activePageUrl])
      this.vNode = this.state[this.activePageUrl] = vue.h(this.activePageComponent);
    else if (this.state[this.activePageUrl])
      this.vNode = this.state[this.activePageUrl];
    else
      this.vNode = vue.h(TezSlot);
    return vue.h(vue.h(vue.KeepAlive, null, this.vNode));
  }
});
...
const TezLink = vue.defineComponent({
  props: ["to", "class"],
  render() {
...
const NODE_NAME = "div";
const TezIterator = vue.defineComponent({
  data() {
    return {
      sourceItems: [],
      isInteractive: false,
  ...
  render() {
    ...
    if (this.$slots && this.$slots.default)
      this.sourceItems.forEach((item, index) => {
        ...
        vNodes.push(vue.h(this.vChildNode || NODE_NAME, { class: this.childClass, key: index }, slot));
      });
    if (this.sourceItems.length > 0)
      js.idleCallback(this.bindNext, { timeout: 2e3 });
    if (!this.lazyRef)
      this.lazyRef = vue.h("div", { ref: "divLazy" }, "");
    vNodes.push(this.lazyRef);
    return vue.h(this.vNode || NODE_NAME, { class: this.class }, vNodes);
  }
});
...
const TezView = vue.defineComponent({
  data() {
    return {
      state: {},
      tezNode: vue.h(TezIndex),
      vNode: void 0,
      layoutName: void 0,
  ...
  render() {
    ...
    if (typeof layoutComponent === "function")
      tezPages.masterPages[this.layoutName] = layoutComponent = vue.defineAsyncComponent(layoutComponent);
    if (!layoutComponent)
      cacheState.layoutVNode[CACHE_KEY_LAYOUT_TEZ_PAGE] = vNode = cacheState.layoutVNode[CACHE_KEY_LAYOUT_TEZ_PAGE] || vue.h(TezPage);
    else if (layoutComponent)
      cacheState.layoutVNode[this.layoutName] = vNode = cacheState.layoutVNode[this.layoutName] || vue.h(layoutComponent);
    return vue.h(vue.h(vue.KeepAlive, null, vNode));
  }
});

function createTezApp(tezAppOptions) {
  componentState.tezAppOptions = tezAppOptions;
  resolvePreCode().then((t) => vue.createApp(TezView).use(tez.register(tezAppOptions)).mount("#tez_app"));
}

exports.createTezApp = createTezApp;
exports.imageLoader = imageLoader;
exports.registerTezPage = registerTezPage;
