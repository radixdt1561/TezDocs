...
function urlReplacer(value, columnName) {
  if (typeof value === "string" && value.indexOf("https://") !== -1) {
    let strapiConfig = common.commonContainer.getStrapiConfig();
...
MarkdownIt.renderer.rules.image = function(tokens, idx, options, env, self) {
  try {
    const imageCdn = common.commonContainer.getStrapiConfig().media.cdnUri;
...
function toCamelCase(value) {
  if (common.commonContainer.getStrapiConfig().serializeJson.isCamelCase)
...
function clear(jsonObject, isRoot) {
  let htmlSanitizer = common.commonContainer.tezConfig.payload.page.htmlSanitizer;
  var jObject = {};
  Object.keys(jsonObject).forEach((t) => {
    var keyName = toCamelCase(t);
    if (jsonObject[t] && common.isObject(jsonObject[t])) {
      if (!ignoreColumns(t)) {
    ...
    } else if (jsonObject[t] && common.isArray(jsonObject[t])) {
      if (!ignoreColumns(t)) {
        jObject[keyName] = [];
...
function ignoreColumns(name, isRoot) {
  let columns = common.commonContainer.getStrapiConfig().ignoreColumns || [];
...
function swapDataPropValue(jObject) {
  let jsonObject = {};
  if (Array.isArray(jObject) && jObject[0] && common.isObject(jObject[0])) {
    let items = [];
    jObject.forEach((item) => {
      items.push(swapDataPropValue(item));
    });
    return items;
  } else if (jObject && common.isObject(jObject)) {
    Object.keys(jObject).forEach((key) => {
      if (Array.isArray(jObject[key]) && jObject[key][0] && common.isObject(jObject[key][0])) {
        let items = [];
      ...
      } else if (common.isObject(jObject[key])) {
        if (key === "data")
...
class RequestService {
  constructor() {
    const { apiUri, userCredential, version } = common.commonContainer.getStrapiConfig();
    this.apiUri = apiUri || DEFAULT_STRAPI_URL;
    this.userCredential = userCredential;
    this.apiString = version === 4 ? "/api" : "";
  }
  async get(uri) {
    const { data } = await axios__default.get(`${this.apiUri}${this.apiString}${uri}`, this.headers);
  ...
  async login() {
    if (this.userCredential) {
      const { data } = await axios__default.post(`${this.apiUri}/admin/login`, this.userCredential);
  ...
  async payloadRequest(uri) {
    const { data } = await axios__default.get(uri, this.headers);
    return this.camalizeData(data.data);
  }
}
...
class InternationalizationService {
  constructor(requestService) {
    ...
    const { enableInternationalization, version } = common.commonContainer.getStrapiConfig();
...
const QUERY_PARAMS = {
  3: {
    limit: {
      "_limit": () => common.commonContainer.getStrapiConfig().limit
    },
    locale: {
      "_locale": (value) => value
    }
  ...
  4: {
    limit: {
      pagination: {
        pageSize: () => common.commonContainer.getStrapiConfig().limit
      }
    },
    locale: {
      "locale": (value) => value
...
function getQueryString(queryParamsObject) {
  return qs__namespace.stringify(queryParamsObject, {
...
function getQueryParams(name, value = "") {
  const strapiVersion = common.commonContainer.getStrapiConfig().version;
...
class PathResolver {
  ...
  get robotsTxtPath() {
    return common.getPath([this.publicFolder, "robots.txt"]);
  }
  get redirectRoutesJsonPath() {
    return this.commonPath.redirectRoutesJsonPath;
  }
  get sitemapJsonPath() {
    return common.getPath([this.rootPath, "sitemap.json"]);
  }
  get globJsonPath() {
    return common.getPath([this.rootPath, "glob.json"]);
  }
  get routeComponentsPath() {
    return common.getPath([this.rootPath, "route-components.json"]);
  }
  get sitemapXmlPath() {
    return common.getPath([this.publicFolder, "sitemap.xml"]);
  }
  get sitemapNotMappedJsonPath() {
    return common.getPath([this.publicFolder, "sitemap-not-mapped.json"]);
  }
  getCollectionPayloadPath(collectionName) {
    return common.getPath([this.payloadPath, "collections", collectionName], true);
  }
  getFilterCollectionPayloadPath(collectionName) {
    return common.getPath([this.payloadPath, "filter-collections", collectionName], true);
  }
  pathExists(path) {
...
class PageRoute {
  constructor(requestService) {
    ...
    const { pageCollectionConfig } = common.commonContainer.getStrapiConfig();
  ...
  addRoute(routeItem, isPage = false) {
    if (this.routes.filter((t) => t.path === routeItem.path).length === 0)
      this.routes.push({
        ...
        path: routeItem.path,
        fPath: getUrl(routeItem.path).split("/").join(path__namespace.sep),
        isPage
      });
  }
  addRoutes(routes) {
...
function getFilterQueryParams(filterJson, populate = "deep") {
  const version = common.commonContainer.getStrapiConfig().version;
  if (version === 4) {
    let jObject = {};
...
function removeAppProps(jObject) {
  ...
  if (Array.isArray(jObject)) {
    ...
    return items;
  } else if (common.isObject(jObject)) {
    Object.keys(jObject).forEach((key) => {
...
function getComponentName(componentName) {
  return common.commonContainer.getStrapiConfig().componentNames ? common.commonContainer.getStrapiConfig().componentNames[componentName] : componentName;
}
...
function getPagePrePostComponents(collection) {
  var result = [];
  if (collection) {
    common.commonContainer.getStrapiConfig();
...
function qsStringify(filterJson) {
  return qs__namespace.stringify(filterJson, {
    encodeValuesOnly: true
  });
}
...
function sourcePaginationByUrl(url, componentName, item) {
  const sourcePaginationByUrl2 = common.commonContainer.getStrapiConfig().optimization?.sourcePaginationByUrl;
  let isPass = false;
...
function getChecksum(text) {
  return crypto__namespace.createHash("md5").update(text).digest("hex");
}

class CollectionIndexer {
  constructor() {
    this.pagination = common.commonContainer.getStrapiConfig().pagination;
    this.pathResolver = new PathResolver();
  }
...
async function dataRequest(queryObject, entity) {
  const entityQueryParams = common.commonContainer.tezConfig?.payload?.entityQueryParams;
...
async function DataResolver(pageContent, url) {
  const moduleOptions = common.commonContainer.getStrapiConfig();
...
async function parseStrapiData(pageContent, url, dynamicData, referenceData) {
  ...
  for (let i = 0; i < genericSection.length; i++) {
    ...
    item = runDataSanitizer(common.commonContainer.tezConfig.payload?.page?.dataSanitizers, item, url, componentName);
    ...
    if (getComponentName(componentName)) {
      ...
      if (collectionType && moduleOptions.version > 3 || (item[FIELD_COMPONENT_TYPE_NAME] === FIELD_COMPONENT_NAME_GET_ALL_DATA || item[FIELD_COMPONENT_TYPE_NAME] === FIELD_COMPONENT_NAME_GET_DATA_FIELD_VALUE || item[FIELD_DATA_TYPE_RESULT] === DATA_CONTROL_ALL || item[FIELD_DATA_TYPE_RESULT] === DATA_CONTROL_FILTER || item[FIELD_DATA_TYPE_RESULT] === DATA_CONTROL_GET_RECORD) && moduleOptions.componentDataFieldSelectors[getComponentName(componentName)]) {
        ...
        queryResult.forEach((dataItem, index) => {
          queryResult[index] = runDataSanitizer(common.commonContainer.tezConfig?.payload?.page?.dataSanitizers, dataItem, url, removeSpace(readProp(dataItem, VUE_COMPONENT_NAME_PROPS)), componentName);
        });
...
class MasterPageCollection {
  ...
  async setMasterPageInfo(uri, filterJson, item) {
    ...
    if (!this.isGenerated && item.masterPage && item.masterPage.title) {
      ...
      const directoryPath = path__namespace.join(this.pathResolver.payloadPath, "master-pages");
      common.createPath(directoryPath);
      let index = 0;
      ...
      let filePath = path__namespace.join(directoryPath, `${replaceSpace(item.masterPage.title)}.json`);
      this.globWriter.addLayout(item.masterPage.layoutName);
      await common.writeFileSync(filePath, { layoutName: item.masterPage.layoutName, slots: this.pageSlot.slots });
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (!pathResolver.pathExists(filePath)) {
        ...
        const writer = fs__namespace.createWriteStream(filePath);
        writer.on("error", (err) => {
        ...
        writer.on("close", () => {
          if (!error) {
            if (common.commonContainer.tezConfig.image?.enableWebPConversion) {
            } else
              resolve(true);
          }
        });
...
async function getImage(imageUrl) {
  return await axios__default({
    method: "get",
    url: `${common.commonContainer.getStrapiConfig().apiUri}${imageUrl}`,
    responseType: "stream"
  });
}
...
class BaseGenerator {
  constructor(redirectRoute, sitemap, globWriter, pageRoute) {
    ...
    const { payload } = common.commonContainer.tezConfig;
    this.pathResolver = new PathResolver();
    this.payload = payload;
  }
  async generatePage(page) {
    const url = getUrl(page.url);
    const directoryPath = path__namespace.join(this.pathResolver.payloadPath, url);
    ...
    for (let j = 0; j < page.components.length; j++) {
      ...
      if (isExits) {
        const filePath2 = path__namespace.join(directoryPath, componentId + ".json");
        ...
        if (this.payload.page.maxPreLoadComponent <= j)
          await common.writeFileSync(filePath2, component.data);
        index++;
      }
    }
    let filePath = path__namespace.join(directoryPath, "tags.json");
    await common.writeFileSync(filePath, cleanObject(page.seo || {}));
    const fileName = url.split("/");
    filePath = path__namespace.join(directoryPath, fileName[fileName.length - 1] + ".json");
    let pageJson = { slots: pageSlot.slots };
    if (page.masterPageName)
      pageJson["masterPage"] = replaceSpace(page.masterPageName);
    await common.writeFileSync(filePath, pageJson);
    if (page.sitemap)
      this.sitemap.add({ sitemap: { ...page.sitemap, ...{ loc: `${common.commonContainer.getStrapiConfig().siteUrl}${page.url}` } } });
...
class PayloadGenerator extends BaseGenerator {
  ...
  async generate(route, dynamicPageRoute) {
    ...
    const directoryPath = path__namespace.join(this.pathResolver.payloadPath, baseUrl);
    const isNotExits = common.createPath(directoryPath);
    if (isNotExits) {
      console.log(baseUrl);
...
class Sitemap {
  constructor() {
    this.sitemap = {};
    this.pathResolver = new PathResolver();
    this.sitemap = common.readFileSync(this.pathResolver.sitemapJsonPath) || {};
  }
  add(page) {
...
class RobotTxtGenerator {
  constructor() {
    this.request = new RequestService();
    this.pathResolver = new PathResolver();
    this.robotsConfig = common.commonContainer.getStrapiConfig().robots;
  }
  async generate() {
    if (this.robotsConfig && !common.commonContainer.getStrapiConfig().customPayloadGenerator) {
    ...
    } else if (!common.commonContainer.tezConfig.robots || common.commonContainer.tezConfig.robots[common.commonContainer.tezConfig.envName]) {
      let text = common.commonContainer.tezConfig.robots && common.commonContainer.tezConfig.robots[common.commonContainer.tezConfig.envName] ? common.commonContainer.tezConfig.robots[common.commonContainer.tezConfig.envName] : `User-agent: *
Disallow: /`;
      this.save(text);
    }
  }
...
class RedirectRoute {
  constructor() {
    this.routes = [];
    this.pathResolver = new PathResolver();
    this.routes = common.readFileSync(this.pathResolver.redirectRoutesJsonPath) || [];
  }
  add(page) {
...
class CustomPagePayload extends BaseGenerator {
  ...
  async generate(routePath) {
    const pages = common.commonContainer.tezConfig.pages;
...
class DeploymentPathResolver {
  ...
  get firebaseConfigPath() {
    return common.getPath([this.commonPath.rootPath, "firebase.json"]);
  }
...
class FirebaseDomain extends BaseDeployment {
  ...
  create(firebase, redirectRoutes) {
    ...
    if (common.isObject(firebase)) {
      firebaseConfig = firebase;
    } else
      firebaseConfig = FIRE_BASE_CONFIG;
    this.addRedirects(firebaseConfig, redirectRoutes);
    common.writeFileSync(this.pathResolver.firebaseConfigPath, firebaseConfig);
  }
...
class DeploymentDomain {
  constructor() {
    ...
    this.commonPath = new common.CommonPathResolver();
    this.redirectRoutes = common.readFileSync(this.commonPath.redirectRoutesJsonPath);
  }
  generate() {
    if (common.commonContainer.tezConfig.deployment) {
      if (common.commonContainer.tezConfig.deployment.firebase)
        this.firebase.create(common.commonContainer.tezConfig.deployment.firebase, this.redirectRoutes);
    }
  }
}
...
class GlobWriter {
  constructor() {
    this.pathResolver = new PathResolver();
    const globJson = common.readFileSync(this.pathResolver.globJsonPath) || { components: [], layouts: [], pages: [] };
  ...
  addComponent(name) {
    const isPathExits = this.pathResolver.pathExists(common.getPath([this.pathResolver.componentsPath, `${name}.vue`], false));
  ...
  write() {
    common.writeFileSync(this.pathResolver.globJsonPath, { components: this.components, layouts: this.layouts, pages: this.pages });
  }
}
...
class PayloadModuleRunner extends BaseGenerator {
  ...
  async runPayload(config) {
    const modules = common.commonContainer.tezConfig.modules;
...
class PageCollection {
  ...
  async generate(routePath) {
    ...
    if (common.commonContainer.tezConfig.strapi) {
      if (common.commonContainer.tezConfig.strapi.customPayloadGenerator)
        await common.commonContainer.tezConfig.strapi.customPayloadGenerator(config);
      else
        await this.generateStrapiPayload(routePath);
    }
    if (common.commonContainer.tezConfig.pages)
      await this.customPagePayload.generate(routePath);
    await this.moduleRunner.runPayload(config);
    this.pageRoute.save();
    if (common.commonContainer.tezConfig.generate.sitemap)
      this.sitemap.save();
    if (common.commonContainer.tezConfig.generate.robots)
...
exports.GlobWriter = GlobWriter;
exports.PageCollection = PageCollection;
exports.getFriendlyComponentName = getFriendlyComponentName;
