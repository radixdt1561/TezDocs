...
function createPath(pathToCreate) {
  ...
  pathToCreate.split(require$$0.sep).reduce((prevPath, folder) => {
    const currentPath = require$$0.join(prevPath, folder, require$$0.sep);
    isNotExits = !fs$9.existsSync(currentPath);
    if (isNotExits) {
      fs$9.mkdirSync(currentPath);
    }
    return currentPath;
  }, "");
  return isNotExits;
...
function getPath$1(pathCollection, isCreateFolder = false) {
  const currentPath = require$$0.join(...pathCollection);
  if (isCreateFolder)
...
const path$7 = require$$0__default;
const scan = scan_1;
const parse = parse_1;
...
const applyBaseToPattern = (pattern, base) => isNegativePattern(pattern)
	? '!' + path$b.posix.join(base, pattern.slice(1))
	: path$b.posix.join(base, pattern);
...
const checkCwdOption = options => {
	...
	let stat;
	try {
		stat = fs$a.statSync(options.cwd);
	} catch {
		return;
	}
...
const createFilterFunction = isIgnored => {
	...
	return fastGlobResult => {
		...
		const pathKey = path$b.normalize(path);
...
class CommonPathResolver {
  ...
  getFilePath(directories, fileName) {
    return require$$0.join(getPath$1(directories), fileName);
  }
...
function readFileSync(path, isString = false) {
  if (existsSync(path)) {
    var content = fs$9.readFileSync(path, "utf-8");
...
function resolvePath(fPath) {
  return require$$0.resolve(process.cwd(), fPath);
}
...
async function writeFileSync(path, content, isString = false) {
  ...
  await fs$9.writeFileSync(path, content);
}
...
function getUrlLastPath(url) {
  ...
  if (url) {
    ...
    lastUrlNode = require$$0.sep + splitUrl[splitUrl.length - 1];
  }
  return lastUrlNode;
}
...
export { CommonPathResolver, PayloadReader, commonContainer, convertDateToTicks, createPath, deepMerge, getPath$1 as getPath, isArray, isObject, readFileSync, resolvePath, subText, writeFileSync };
