...
const fs$a = require('node:fs');
const path$b = require('node:path');
...
const fs__namespace = /*#__PURE__*/_interopNamespace(fs$9);
const fs__default = /*#__PURE__*/_interopDefaultLegacy(fs$9);
const require$$0__namespace = /*#__PURE__*/_interopNamespace(require$$0);
const require$$0__default$1 = /*#__PURE__*/_interopDefaultLegacy(require$$0);
const fs__default$1 = /*#__PURE__*/_interopDefaultLegacy(fs$a);
const path__default = /*#__PURE__*/_interopDefaultLegacy(path$b);
...
const CACHE_PATH = ".tez";
const DEFAULT_STRAPI_URL = "http://localhost:1337";

function createPath(pathToCreate) {
  var isNotExits = false;
  pathToCreate.split(require$$0__namespace.sep).reduce((prevPath, folder) => {
    const currentPath = require$$0__namespace.join(prevPath, folder, require$$0__namespace.sep);
...
function getPath$1(pathCollection, isCreateFolder = false) {
  const currentPath = require$$0__namespace.join(...pathCollection);
  if (isCreateFolder)
...
const events_1 = require$$0__default$3;
...
const applyBaseToPattern = (pattern, base) => isNegativePattern(pattern)
	? '!' + path__default.posix.join(base, pattern.slice(1))
	: path__default.posix.join(base, pattern);

const parseIgnoreFile = (file, cwd) => {
	const base = slash(path__default.relative(cwd, path__default.dirname(file.filePath)));
...
const toRelativePath = (fileOrDirectory, cwd) => {
	cwd = slash(cwd);
	if (path__default.isAbsolute(fileOrDirectory)) {
		if (slash(fileOrDirectory).startsWith(cwd)) {
			return path__default.relative(cwd, fileOrDirectory);
		}
...
const normalizeOptions$1 = (options = {}) => ({
	cwd: toPath(options.cwd) || process__default.cwd(),
});
...
const isIgnoredByIgnoreFiles = async (patterns, options) => {
	...
	const files = await Promise.all(
		paths.map(async filePath => ({
			filePath,
			content: await fs__default$1.promises.readFile(filePath, 'utf8'),
		})),
	);
...
const checkCwdOption = options => {
	...
	let stat;
	try {
		stat = fs__default$1.statSync(options.cwd);
	} catch {
		return;
	}
...
const createFilterFunction = isIgnored => {
	...
	return fastGlobResult => {
		...
		const pathKey = path__default.normalize(path);
...
class CommonPathResolver {
  get indexHtmlPath() {
    return getPath$1([commonContainer.buildOptions.rootDir, "index.html"], false);
  }
  get sourceCodePath() {
    return getPath$1([this.rootPath, commonContainer.tezConfig.sourceCodePath], true);
  }
  get rootPath() {
    return getPath$1([commonContainer.buildOptions.rootDir], true);
  }
  get publicFolder() {
    return getPath$1([commonContainer.buildOptions.rootDir, "public"], true);
  }
  get cachePath() {
    return getPath$1([commonContainer.buildOptions.rootDir, CACHE_PATH], true);
  }
  get dynamicRoutesPath() {
    return getPath$1([this.depsPath, "dynamic-routes.js"], false);
  }
  get depsPath() {
    return getPath$1([this.cachePath, "deps"], true);
  }
  get distPath() {
    return getPath$1([this.rootPath, "dist"], false);
  }
  get clientEnvPath() {
    return getPath$1([this.envPath, "client.ts"]);
  }
  get tezTsPath() {
    return getPath$1([this.cachePath, "tez.ts"]);
  }
  get serverEnvPath() {
    return getPath$1([this.envPath, "server.ts"]);
  }
  get addLibPath() {
    return getPath$1([commonContainer.buildOptions.rootDir, commonContainer.tezConfig.sourceCodePath, "add-lib.ts"], false);
  }
  get routesJsonPath() {
    return getPath$1([this.cachePath, "routes.json"]);
  }
  get redirectRoutesJsonPath() {
    return getPath$1([this.cachePath, "redirect-routes.json"]);
  }
  get payloadPath() {
    return getPath$1([this.publicFolder, "payload"], true);
  }
  get imageFolderPath() {
    getPath$1([this.publicFolder, "uploads"], true);
    return this.publicFolder;
  }
  get storePath() {
    return getPath$1([commonContainer.buildOptions.rootDir, "store", "index.ts"], false);
  }
  get routerPath() {
    return getPath$1([commonContainer.buildOptions.rootDir, "router", "index.ts"], false);
  }
  get layoutsPath() {
    return getPath$1([commonContainer.buildOptions.rootDir, commonContainer.tezConfig.sourceCodePath, "layouts"], false);
  }
  get componentsPath() {
    return getPath$1([commonContainer.buildOptions.rootDir, commonContainer.tezConfig.sourceCodePath, "components"], false);
  }
  get pluginsPath() {
    return getPath$1([commonContainer.buildOptions.rootDir, commonContainer.tezConfig.sourceCodePath, "plugins", "index.ts"], false);
  }
  get pagesPath() {
    return getPath$1([commonContainer.buildOptions.rootDir, commonContainer.tezConfig.sourceCodePath, "pages"], false);
  }
  get tzJsPath() {
    return getPath$1([this.distPath, "tz.js"], false);
  }
  get preScriptName() {
  ...
  get envPath() {
    return getPath$1([this.sourceCodePath, "env"], true);
  }
  pathExists(filePath) {
  ...
  getPath(path2) {
    return getPath$1(path2, true);
  }
  getFilePath(directories, fileName) {
    return require$$0__namespace.join(getPath$1(directories), fileName);
  }
  async getFiles(folderName, extension) {
    ...
    return paths.map((path2) => {
      return path2.split(`${folderName}/`)[1].replace(`.${extension}`, "");
    });
  }
}

function readFileSync(path, isString = false) {
  if (fs$9.existsSync(path)) {
...
function resolvePath(fPath) {
  return require$$0__namespace.resolve(process.cwd(), fPath);
}
...
function getUrlLastPath(url) {
  ...
  if (url) {
    ...
    lastUrlNode = require$$0__namespace.sep + splitUrl[splitUrl.length - 1];
  }
  return lastUrlNode;
}
...
class PayloadReader {
  ...
  get componentPath() {
    return getPath$1([this.commonPath.payloadPath, this.route.fPath, `${this.lastUrlPath}.json`]);
  }
  get masterPagePath() {
    return getPath$1([this.commonPath.payloadPath, "master-pages", `${this.components.masterPage.replace(/ /g, "-").toLowerCase()}.json`]);
  }
  get tagsPath() {
    return getPath$1([this.commonPath.payloadPath, this.route.fPath, "tags.json"]);
  }
  get components() {
  ...
  getData(componentName) {
    let componentDataPath = getPath$1([this.commonPath.payloadPath, this.route.fPath, `${componentName}.json`]);
    let data = {};
...
exports.convertDateToTicks = convertDateToTicks;
